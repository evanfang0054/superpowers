...
#
# Sync this agent-harness checkout → prime-radiant-inc/openai-codex-plugins.
...
#   ./scripts/sync-to-codex-plugin.sh -n, --dry-run               # dry run
...
# plugins/agent-harness/ when absent, then copies the tracked plugin files from
...
DEFAULT_BASE="main"
DEST_REL="plugins/agent-harness"
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/agent-harness-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/agent-harness-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/agent-harness/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap agent-harness v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the agent-harness plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/agent-harness/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\`, \`assets/\`, and \`hooks/\`.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Upstream commit: https://github.com/obra/agent-harness/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync agent-harness v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from agent-harness upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: https://github.com/obra/agent-harness/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
