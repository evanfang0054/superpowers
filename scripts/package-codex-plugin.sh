#!/usr/bin/env bash
#
# Package the agent-harness Codex plugin as a rootless archive for portal upload.
#
...
Options:
  --output PATH            Write archive to PATH.
                           Default: ../_tmp/agent-harness-codex-packaging/agent-harness-VERSION.zip
  ...
  --metadata-source PATH   Prior official package directory, .zip, or .tar.gz used to
                           ...
                           Default: ../_tmp/agent-harness-codex-packaging/agent-harness,
                           falling back to agent-harness.zip, then agent-harness.tar.gz
...
The archive is rootless: .codex-plugin/, assets/, hooks/, skills/, README.md,
and LICENSE sit at the archive root. Source-only repo files, tests,
...
command -v "$TAR" >/dev/null || die "tar ($TAR) not found in PATH"
...
[[ -d "$REPO_ROOT/.git" || -f "$REPO_ROOT/.git" ]] || die "repo root is not a git checkout: $REPO_ROOT"
...
if [[ -z "$METADATA_SOURCE" ]]; then
  if [[ -d "$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness"
  elif [[ -f "$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness.zip" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness.zip"
  elif [[ -f "$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness.tar.gz" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness.tar.gz"
  else
    die "no metadata source found; pass --metadata-source <prior package dir, zip, or tar.gz>"
  fi
fi

WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/agent-harness-codex-package.XXXXXX")"
...
if [[ -z "$OUTPUT" ]]; then
  case "$FORMAT" in
    zip)
      OUTPUT="$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness-$VERSION.zip"
      ;;
    tar.gz)
      OUTPUT="$REPO_ROOT/../_tmp/agent-harness-codex-packaging/agent-harness-$VERSION.tar.gz"
      ;;
  esac
fi
...
case "$FORMAT" in
  zip)
    ...
    # Set TZ=UTC for both touch and zip so the stored local-time
    # representation is midnight regardless of the builder's timezone.
...
unexpected_paths="$(
  printf '%s\n' "$archive_paths" |
    grep -E '(^agent-harness/|^\.agents/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)' || true
)"
