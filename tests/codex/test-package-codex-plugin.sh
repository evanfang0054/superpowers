...
# Allow tar binary override for cross-platform testing (bsdtar vs GNU tar)
TAR="${TAR_BIN:-tar}"
...
archive="$TEST_ROOT/agent-harness"
tar_archive="$TEST_ROOT/agent-harness.tar.gz"
...
assert_equals "$source_hooks" "./hooks/hooks-codex.json" "source Codex manifest points to Codex hooks config"

# === Static assertions: script preserves both GNU tar and bsdtar branches ===
script_content="$(cat "$SCRIPT_UNDER_TEST")"
assert_contains "$script_content" "--owner=:0 --group=:0 --numeric-owner" "script contains GNU tar branch"
assert_contains "$script_content" "--uid 0 --gid 0 --uname '' --gname ''" "script contains bsdtar branch"
...
unexpected_pattern='(^agent-harness/|^\.agents/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)'
...
archive_icon="$(printf '%s\n' "$archive_paths" | grep '^assets/.*\.svg$' | head -1)"
if [[ -n "$archive_icon" ]]; then
  pass "archive includes composer icon ($archive_icon)"
else
  fail "archive includes composer icon"
fi
...
assert_equals "$manifest_summary" "agent-harness	$expected_version	./skills/	$source_hooks" "archive manifest preserves source hooks"
...
# === Hooks path preservation assertion ===
hooks_value_in_archive="$(python3 -c "
import json
data = json.load(open('${extracted}/.codex-plugin/plugin.json'))
hooks = data.get('hooks')
print(hooks if isinstance(hooks, str) else '')
")"
if [[ -n "$hooks_value_in_archive" ]]; then
  hooks_rel="${hooks_value_in_archive#./}"
  if [[ -f "$extracted/$hooks_rel" ]]; then
    pass "archive preserves hooks file at $hooks_value_in_archive"
  else
    fail "archive preserves hooks file at $hooks_value_in_archive (not found: $extracted/$hooks_rel)"
  fi
else
  pass "archive manifest has no hooks path to verify (hooks: {})"
fi
...
# === Tar archive member metadata assertions (uid/gid/mtime) ===
tar_metadata_uid_gid_mtime="$(python3 - "$tar_archive" <<'PY'
import sys, tarfile
with tarfile.open(sys.argv[1]) as archive:
    ...
    print(("uids=" + str(sorted(uids))), ("gids=" + str(sorted(gids))), ("mtimes=" + str(sorted(mtimes))))
PY
)"
assert_contains "$tar_metadata_uid_gid_mtime" "uids=[0]" "tar.gz archive normalizes entry uids"
assert_contains "$tar_metadata_uid_gid_mtime" "gids=[0]" "tar.gz archive normalizes entry gids"
assert_contains "$tar_metadata_uid_gid_mtime" "mtimes=[0]" "tar.gz archive normalizes entry mtimes"

metadata_archive="$TEST_ROOT/metadata-source.tar.gz"
metadata_zip="$TEST_ROOT/metadata-source.zip"
archive_from_tar_source="$TEST_ROOT/agent-harness-from-tar-source.zip"
archive_from_zip_source="$TEST_ROOT/agent-harness-from-zip-source.zip"
(
  cd "$metadata_source"
...
# Copy the script separately in case it is untracked (not yet committed)
