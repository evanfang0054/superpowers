...
export class OrderService {
  ...
  async cancel(userId: number, id: number) {
    return this.lifecycleService.cancel(userId, id, (uid, oid) => this.findOne(uid, oid));
  }

  /** 支付 */
  async pay(userId: number, id: number) {
    return this.lifecycleService.pay(userId, id, (uid, oid) => this.findOne(uid, oid));
  }

  /** 发货 (Admin) */
  async ship(id: number, dto: ShipDto) {
    return this.lifecycleService.ship(id, dto, (oid) => this.findOneInternal(oid));
  }

  /** 确认收货 */
  async confirm(userId: number, id: number) {
    return this.lifecycleService.confirm(userId, id, (uid, oid) => this.findOne(uid, oid));
  }

  /** 申请退款 */
  async requestRefund(userId: number, id: number, dto: RefundRequestDto) {
    return this.lifecycleService.requestRefund(userId, id, dto, (uid, oid) =>
