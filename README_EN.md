...
This project uses [Changesets](https://github.com/changesets/changesets) for version management. The version number is kept in sync across 4 files: `package.json`, `.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`, and `.claude-plugin/marketplace.json`.
...
1. **Add a changeset** describing the change:
   ...
   Interactive prompt: select `agent-harness` → choose `minor` / `major` / `patch` → write a one-line summary. This creates a `.changeset/*.md` file.
...
3. **Run the release** — bumps the version, generates the CHANGELOG entry, and syncs the 3 plugin manifests:

   ```bash
   pnpm release
   ```

   This runs `changeset version` (bumps `package.json` + updates `CHANGELOG.md` with GitHub PR links and contributor credits + consumes the changeset file) followed by `./scripts/sync-plugin-versions.sh` (propagates the new version to the 3 plugin manifests).

4. **Verify all 4 files are in sync:**

   ```bash
   ./scripts/sync-plugin-versions.sh --check
   ```

   Exits `0` when in sync, `2` when drift is detected.
...
| Command | Purpose |
|---------|---------|
| `pnpm changeset` | Author a change description (before merging a feature) |
| `pnpm release` | Consume changesets → bump version + changelog + sync manifests |
| `./scripts/sync-plugin-versions.sh --check` | Detect version drift across the 4 files |

The `CHANGELOG.md` is auto-generated by `@changesets/changelog-github`. Historical releases (pre-changesets, 64 versions) are preserved at the bottom of `CHANGELOG.md`. The package is marked `private` — versions are tracked in git + the 4 manifests only, not published to npm.
